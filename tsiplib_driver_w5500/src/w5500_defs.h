...
typedef struct {
        uint8_t socket_register;
        uint8_t socket_tx_buffer;
        uint8_t socket_rx_buffer;
} w5500_bsb_socket_t;
...
#define FT_W5500_SET_COMMON_16(__name__)             \
        int w5500_set_##__name__(uint16_t data) { \
...
#define FT_W5500_SET_COMMON_BA4(__name__)             \
        int w5500_set_##__name__(uint8_t* data) { \
...
#define FT_W5500_GET_COMMON_TP8(__name__,__type__)             \
        ...
        w5500_read_byte(W5500_ADDR_COMMON_##__name__, _bsb.common_register, FORCE_CAST(data, uint8_t*)); \
                return W5500_OK; \
        }

#define FT_W5500_SET_COMMON_TP8(__name__,__type__)             \
        int w5500_set_##__name__(__type__ data) { \
        w5500_write_byte(W5500_ADDR_COMMON_##__name__, _bsb.common_register, FORCE_CAST(data, uint8_t)); \
                return W5500_OK; \
        }

#define FT_W5500_GET_SOCKET_8(__name__)             \
        int w5500_get_socket_##__name__(uint8_t socketNumber, uint8_t* data) { \
...
#define FT_W5500_SET_SOCKET_8(__name__)             \
        int w5500_set_socket_##__name__(uint8_t socketNumber, uint8_t data) { \
                CHECK_SOCKET_NUMBER(socketNumber); \
...
#define FT_W5500_GET_SOCKET_16(__name__)             \
        int w5500_get_socket_##__name__(uint8_t socketNumber, uint16_t* data) { \
                CHECK_SOCKET_NUMBER(socketNumber); \
                uint8_t temp = 0; \
...
#define FT_W5500_SET_SOCKET_16(__name__)             \
        int w5500_set_socket_##__name__(uint8_t socketNumber, uint16_t data) { \
                CHECK_SOCKET_NUMBER(socketNumber); \
